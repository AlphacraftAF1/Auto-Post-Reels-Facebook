    ...
    try:
        ...
        for media_info in media_to_process_this_run:
            ...
            try:
                ...
                is_reel = False # is_reel hanya untuk logging/informasi, tidak lagi untuk memilih endpoint
                if media_type == 'video':
                    logging.info("Menganalisis video untuk deteksi Reels...")
                    is_reel = video_utils.is_reel(media_path)
                    if is_reel:
                        logging.info("Video dideteksi sebagai Reels (akan diunggah via endpoint video biasa).")
                    else:
                        logging.info("Video dideteksi sebagai Video Reguler (akan diunggah via endpoint video biasa).")
                else:
                    logging.info("Media dideteksi sebagai Foto.")

                # 5. Upload ke Facebook
                # Sekarang, semua video diunggah melalui upload_video(), Facebook akan otomatis mendeteksi Reels.
                logging.info(f"Mengunggah media ke Facebook sebagai {'Video' if media_type == 'video' else 'Photo'}...")
                ...
                if post_id:
                    ...
                    send_telegram_notification(
                        ...
                        f"Tipe: {'Reels (via video endpoint)' if is_reel else media_type.capitalize()}\n" # Perbarui notifikasi
